...
module.exports = {
	...
	registerLambdaTasks: function (grunt, names, deployTasks) {
		...
		grunt.registerTask('deploy-api', _.flatten(['nodeunit:api', _.map(names, function (name) {
				return 'deploy-' + name;
			}), 'deploy-api-definition', deployTasks]));
	}
}